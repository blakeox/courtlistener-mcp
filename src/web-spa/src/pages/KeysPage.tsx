...
export function KeysPage(): React.JSX.Element {
  ...
  const [previousActiveKey, setPreviousActiveKey] = React.useState<{
    id: string;
    label: string;
  } | null>(null);
  const [revokePreviousKey, setRevokePreviousKey] = React.useState<{
    id: string;
    label: string;
  } | null>(null);
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      const expiresDays =
        expiryPreset === 'custom' ? parsedCustom : Number.parseInt(expiryPreset, 10);
      return createKey(
        {
  ...
  return (
    <div className={stackClass}>
      <div className={twoColClass}>
        ...
        <Card
          title="Create rotation key"
          subtitle="One-time token reveal with guided promotion and optional revoke."
        >
          ...
          <Button
            onClick={() => {
              ...
              if (!warned) {
                ...
                toast(
                  'Token saved to localStorage — any script on this page can read it. Use Account page to clear.',
                  'info',
                );
              }
              if (prior) {
      ...
      <Modal
        open={Boolean(revokePreviousKey)}
