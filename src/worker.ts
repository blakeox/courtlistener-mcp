...
import { getServerInfo } from './infrastructure/protocol-constants.js';
...
type WorkerEnv = Record<string, unknown> & {
  ...
  MCP_SSE_AUTH_TOKEN?: string;
  OIDC_ISSUER?: string;
  OIDC_AUDIENCE?: string;
  OIDC_JWKS_URL?: string;
  OIDC_REQUIRED_SCOPE?: string;
  MAX_SSE_CONNECTIONS?: string | number;
  MAX_SSE_CONNECTIONS_PER_IP?: string | number;
};
...
function envStr(env: WorkerEnv, key: keyof WorkerEnv): string | undefined {
  const value = env[key];
  return typeof value === 'string' && value.trim() !== '' ? value : undefined;
}

function envNum(env: WorkerEnv, key: keyof WorkerEnv): number | undefined {
  const value = env[key];
...
function applyRuntimeConfig(env: WorkerEnv): { hasCourtListenerApiKey: boolean } {
  const gg = g();
  const sseAuthToken = envStr(env, 'SSE_AUTH_TOKEN') || envStr(env, 'MCP_SSE_AUTH_TOKEN');
  const courtListenerApiKey = envStr(env, 'COURTLISTENER_API_KEY');

  if (sseAuthToken) gg.__SSE_AUTH_TOKEN__ = sseAuthToken;
  else delete gg.__SSE_AUTH_TOKEN__;

  const oidc: OidcCfg = {};
  const issuer = envStr(env, 'OIDC_ISSUER');
  const audience = envStr(env, 'OIDC_AUDIENCE');
  const jwksUrl = envStr(env, 'OIDC_JWKS_URL');
  const requiredScope = envStr(env, 'OIDC_REQUIRED_SCOPE');
  if (issuer) oidc.issuer = issuer;
  if (audience) oidc.audience = audience;
  if (jwksUrl) oidc.jwksUrl = jwksUrl;
  if (requiredScope) oidc.requiredScope = requiredScope;
  gg.__OIDC = oidc;

  const limits: LimitsCfg = {};
  const maxTotal = envNum(env, 'MAX_SSE_CONNECTIONS');
  const maxPerIp = envNum(env, 'MAX_SSE_CONNECTIONS_PER_IP');
  if (maxTotal !== undefined) limits.maxTotal = maxTotal;
  if (maxPerIp !== undefined) limits.maxPerIp = maxPerIp;
  gg.__LIMITS = limits;

  if (courtListenerApiKey && typeof process !== 'undefined') {
    process.env.COURTLISTENER_API_KEY = courtListenerApiKey;
  }

  return { hasCourtListenerApiKey: Boolean(courtListenerApiKey) };
}
...
async function handleMCPRequest(request: Request): Promise<Response> {
  ...
  const oidcAuthConfig = oidcConfig?.issuer
    ? {
        issuer: oidcConfig.issuer,
        ...(oidcConfig.audience !== undefined && { audience: oidcConfig.audience }),
        ...(oidcConfig.jwksUrl !== undefined && { jwksUrl: oidcConfig.jwksUrl }),
        ...(oidcConfig.requiredScope !== undefined && { requiredScope: oidcConfig.requiredScope }),
      }
    : undefined;
  const staticToken = g().__SSE_AUTH_TOKEN__;
  const authMiddleware = new UnifiedAuthMiddleware({
    ...(oidcAuthConfig !== undefined && { oidc: oidcAuthConfig }),
...
export default {
  async fetch(request: Request, env: WorkerEnv, _ctx: unknown): Promise<Response> {
    try {
      const url = new URL(request.url);
      const runtime = applyRuntimeConfig(env);
      ...
      if (!runtime.hasCourtListenerApiKey && url.pathname === '/' && request.method === 'POST') {
        return new Response('Server misconfigured: missing COURTLISTENER_API_KEY', {
          status: 500,
          headers: {
            'Content-Type': 'text/plain',
            'Access-Control-Allow-Origin': '*',
