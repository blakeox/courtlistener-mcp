...
async function analyzeComponents(): Promise<void> {
  ...
  const testedComponents = allComponents.filter((comp) => isComponentTested(comp.name, testFiles));
  ...
  if (allComponents.length > 0) {
    console.log(
      `📈 Test Coverage: ${Math.round((testedComponents.length / allComponents.length) * 100)}%\n`,
    );
  }
