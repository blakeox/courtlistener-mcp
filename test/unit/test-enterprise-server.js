...
describe('🏢 Enterprise MCP Server Testing', () => {
  ...
  describe('📊 Performance and Monitoring', () => {
    it('should track request performance', () => {
      const performanceCall = MCPMessageFactory.createCallToolMessage('search_opinions', {
    ...
    it('should handle load balancing scenarios', () => {
      ...
      const loadBalancedCalls = Array.from({ length: 5 }, (_, i) =>
        MCPMessageFactory.createCallToolMessage('search_opinions', { query: `load-test-${i}`, priority: i % 3 }, i + 1)
      );
    ...
    it('should support health check integration', () => {
      const healthCheckCall = MCPMessageFactory.createCallToolMessage('health_check', {
    ...
    it('should handle circuit breaker scenarios', () => {
      const circuitBreakerCall = MCPMessageFactory.createCallToolMessage('search_opinions', {
        query: 'test',
        timeout: 5000,
        retry_count: 3,
      });
    ...
    it('should track error patterns', () => {
      ...
      errorScenarios.forEach((scenario, index) => {
        const errorCall = MCPMessageFactory.createCallToolMessage('search_opinions', scenario, index + 1);
  ...
  describe('🌐 Enterprise Integration Features', () => {
    it('should support batch operations', () => {
      const batchCall = MCPMessageFactory.createCallToolMessage('batch_search', {
        queries: ['constitutional law', 'corporate governance', 'intellectual property'],
        batch_size: 3,
      });
    ...
    it('should handle data export requirements', () => {
      const exportCall = MCPMessageFactory.createCallToolMessage('export_data', {
        format: 'json',
    ...
    it('should support webhook notifications', () => {
      const webhookCall = MCPMessageFactory.createCallToolMessage('setup_webhook', {
    ...
    it('should handle multi-tenant isolation', () => {
      const tenantCall = MCPMessageFactory.createCallToolMessage('search_opinions', {
    ...
    it('should support compliance reporting', () => {
      const complianceCall = MCPMessageFactory.createCallToolMessage('generate_compliance_report', {
