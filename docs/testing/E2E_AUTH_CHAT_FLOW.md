...
- `E2E_TURNSTILE_TOKEN` required only if Turnstile is enforced in your deployment
...
- User confirmation is done through Supabase Admin API to avoid manual email-clicking in automated runs.
- If you need to verify actual email delivery/click paths, pair this with a mailbox-based test harness.
